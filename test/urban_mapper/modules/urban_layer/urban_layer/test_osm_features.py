...
class TestOSMFeatures:
  ...
  def test_from_bbox(self):
    assert self.layer.from_bbox(self.bbox, tags={"leisure": ["park", "garden"]}) is None

  def test_from_point(self):
    assert self.layer.from_point(self.point, tags={"building": True}, dist=50.0) is None

  def test_from_polygon(self):
    assert self.layer.from_polygon(self.polygon, tags={"leisure": ["park", "garden"]}) is None
