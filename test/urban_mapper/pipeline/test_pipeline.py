...
class TestUrbanPipeline:
  ...
  layer  = um.UrbanMapper().urban_layer.with_type("streets_roads").from_place(place, network_type="drive").with_mapping(geometry_column="geometry", output_column="mapping_output").build()

  loader1 = um.UrbanMapper().loader.from_file(neighborhoods_path).with_columns(geometry_column="geometry").build()
  loader2 = um.UrbanMapper().loader.from_file(education_path).with_columns(geometry_column="geometry").with_crs(("EPSG:2263", "EPSG:4326")).build()  

  imputer = um.UrbanMapper().imputer.with_type("SimpleGeoImputer").on_columns(geometry_column="geometry").build()

  filter = um.UrbanMapper().filter.with_type("BoundingBoxFilter").build()

  enricher1 = um.UrbanMapper().enricher.with_data(group_by="mapping_output", values_from="gini", data_id="loader1").aggregate_by(method="min", output_column="min_out").build()
  enricher2 = um.UrbanMapper().enricher.with_data(group_by="mapping_output", values_from="gini", data_id="loader1").aggregate_by(method="max", output_column="max_out").build()
  enricher3 = um.UrbanMapper().enricher.with_data(group_by="mapping_output", values_from="population", data_id="loader2").aggregate_by(method="sum", output_column="sum_out").build()
