...
# https://pysal.org/spreg/notebooks/13_ML_estimation_spatial_lag.html
# Maximum Likelihood estimation of the spatial Lag mode
class ML_Lag_Adapter(RegressorMixin, BaseEstimator):
    ...
    def fit(self, X, y, geometry, **kwargs):
        ...
        """
        # Avoids that the ML_Lag discard constant feature columns
        ...
        # Explicitly includes a constant column
        X["CONSTANT"] = 1
        X = X[["CONSTANT"] + X.columns[:-1].tolist()]
        ...
        W = libpysal.weights.Queen.from_dataframe(
            gpd.GeoDataFrame(geometry), use_index=True
        )
    ...
    def predict(self, X, geometry, **kwargs):
        ...
        train_test_geometry = gpd.GeoDataFrame(
            pd.concat([self.__train_geometry, geometry], ignore_index=True)
        )

        # Spatial weights with Queen contiguity (neighbors that share a border or vertex)
        W_train_test = libpysal.weights.Queen.from_dataframe(
            train_test_geometry, use_index=True
        )
        ...
        W_train_test = W_train_test[
            n_train:n_total, 0:n_train
        ...
        spatial_part = W_train_test.dot(
            self.__train_y.flatten()
        )  # shape (n_test,)  (row-standardized weighted sum of __train_y)
...
class CustomModel_Adapter(BaseEstimator):
    ...
    def set_prediction(
        self, X, y_predicted, mask_outlier, y_predicted_column, **kwargs
    ):
        pass
