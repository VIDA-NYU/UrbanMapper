...
def extract_class_args(class_, kwargs):
    class_signature = get_class_args(class_)

    return {
        arg: kwargs[arg]
        for arg in class_signature
        if arg in kwargs
    }
        ...
        **kwargs,
        ...
        if model is None:
            ...
            if target_column is None:
                class_kwargs = extract_class_args(KMeans, kwargs)

                # Sturge's rule
                n_clusters = int(1.0 + 3.322 * np.log10(input_geodataframe.shape[0]))
                class_kwargs.setdefault("n_clusters", n_clusters)
                class_kwargs.setdefault("random_state", random_state)
                model = KMeans(**class_kwargs)

            # Classification
            elif isinstance(
                input_geodataframe[target_column].iloc[0], (np.integer, str)
            ):
                class_kwargs = extract_class_args(SVC, kwargs)

                class_kwargs.setdefault("kernel", "linear")
                class_kwargs.setdefault("max_iter", 1000)
                class_kwargs.setdefault("verbose", False)
                class_kwargs.setdefault("random_state", random_state)
                class_kwargs.setdefault("probability", False)
                model = SVC(**class_kwargs)

            # Regression
            elif longitude_column is not None and latitude_column is not None:
                model = Kriging_Adapter(**kwargs)
            elif geometry_column is not None or (
                hasattr(input_geodataframe, "active_geometry_name")
                and input_geodataframe.active_geometry_name is not None
            ):
                model = ML_Lag_Adapter(**kwargs)
            else:
                class_kwargs = extract_class_args(SVR, kwargs)

                class_kwargs.setdefault("kernel", "linear")
                class_kwargs.setdefault("max_iter", 1000)
                class_kwargs.setdefault("verbose", False)
                model = SVR(**class_kwargs)

        elif isinstance(model, str):
            original_class_kwargs = get_class_args(eval(model))
            class_kwargs = extract_class_args(eval(model), kwargs)

            if "random_state" in original_class_kwargs:
              class_kwargs.setdefault("random_state", random_state)

            model = eval(model)(**class_kwargs)

        if isinstance(model, ClusterMixin) or (
            isinstance(model, Pipeline)
            and np.any([isinstance(stp, ClusterMixin) for stp in model.steps])
        ):
            self.__task_type = ModelTask.CLUSTERING
        elif isinstance(model, ClassifierMixin) or (
            isinstance(model, Pipeline)
            and np.any([isinstance(stp, ClassifierMixin) for stp in model.steps])
        ):
            self.__task_type = ModelTask.CLASSIFICATION
        elif isinstance(model, RegressorMixin) or (
            isinstance(model, Pipeline)
            and np.any([isinstance(stp, RegressorMixin) for stp in model.steps])
        ):
            self.__task_type = ModelTask.REGRESSION

        if weight_path is not None:
            if isinstance(
                model, (ClusterMixin, ClassifierMixin, RegressorMixin, Pipeline)
            ):
...
class ModelBase(ABC, BaseEstimator):
    ...
    def __init__(
        ...
        self.feature_scaler = (
            ...
            if feature_scaler in (None, True)
            else feature_scaler
        )
    ...
    def __extract_target(
        self, input_geodataframe: Union[gpd.GeoDataFrame, pd.DataFrame]
    ) -> Tuple[Optional[Union[gpd.GeoDataFrame, pd.DataFrame]], Optional[pd.Series]]:
        """
    ...
    def __extract_spatial_features(
        self, input_geodataframe: Union[gpd.GeoDataFrame, pd.DataFrame]
    ) -> Tuple[
        ...
        if (
            isinstance(input_geodataframe, gpd.GeoDataFrame)
            and not has_geometry
            and not has_lat_and_long
        ):
            raise ValueError(
                "ModelBase requires latitude and longitude columns or only geometry column"
            )

        return input_geodataframe, long_lat, geometry

    def __remove_target_outliers(
        self,
        input_geodataframe: Union[gpd.GeoDataFrame, pd.DataFrame],
        long_lat: Optional[np.ndarray],
        ...
        """
        if (
            self.__auto_config.task_type == ModelTask.REGRESSION
            and self.target_filter
            and target is not None
        ):
            ## 1 for inliers, -1 for outliers.
            self.mask_outlier = (
                self.target_filter.fit_predict(target) == 1
                if fit
                else self.target_filter.predict(target) == 1
            )
    ...
    def __map_features(
        ...
        if self.feature_mapper and not self.__in_fit_predict:
            input_geodataframe = (
                self.feature_mapper.fit_transform(input_geodataframe)
                if fit
                else self.feature_mapper.transform(input_geodataframe)
            )
    ...
    def __scale_features(
        ...
        if self.feature_scaler:
            columns = (
                input_geodataframe.columns
                if self.ignore_feature_on_scaler is None
                else [
                    col
                    for col in input_geodataframe.columns
                    if col not in self.ignore_feature_on_scaler
                ]
            )
            input_geodataframe.loc[:, columns] = (
                self.feature_scaler.fit_transform(input_geodataframe.loc[:, columns])
                if fit
                else self.feature_scaler.transform(input_geodataframe.loc[:, columns])
            )
    ...
    def __set_target_transform(self):
        ...
        if not isinstance(self.model, Pipeline):
            if self.__auto_config.task_type == ModelTask.REGRESSION:
                self.target_filter = (
                    IQROutlierDetector()
                    if self.target_filter in (None, True)
                    else self.target_filter
                )
                self.target_scaler = (
                    preprocessing.StandardScaler()
                    if self.target_scaler in (None, True)
                ...
                if self.__auto_config.task_type == ModelTask.CLASSIFICATION:
                    self.target_encoder = (
                        ...
                        if self.target_encoder in (None, True)
    ...
    def __create_model_wrapper(self):
        ...
        """
        if self.__auto_config.task_type == ModelTask.REGRESSION:
            if self.weight_path is None and self.target_scaler:
                self.model = TransformedTargetRegressor(
                    regressor=self.model, transformer=self.target_scaler
                )
            if self.weight_path is not None and isinstance(
                self.model, TransformedTargetRegressor
            ):
                self.target_scaler = self.model.transformer

        if self.__auto_config.task_type == ModelTask.CLASSIFICATION:
            if self.weight_path is None and self.target_encoder:
                self.model = TransformedTargetClassifier(
                    classifier=self.model, transformer=self.target_encoder
                )
            if self.weight_path is not None and isinstance(
                self.model, TransformedTargetClassifier
            ):
    ...
    def __train_val_test_split(
        ...
        if self.validation_size:
            ...
            split_kwargs["test_size"] = (
                len(input_geodataframe) / len(train) * self.validation_size
    ...
    def transform(
        self, input_geodataframe: Union[gpd.GeoDataFrame, pd.DataFrame], fit=True
    ) -> Tuple[
        Optional[Union[gpd.GeoDataFrame, pd.DataFrame, np.ndarray]],
        Optional[np.ndarray],
        Optional[gpd.GeoSeries],
        ...
        if (
            ...
            or self.__auto_config.task_type == ModelTask.OTHERS
        ):
            return input_geodataframe, long_lat, geometry, target

        input_geodataframe, long_lat, geometry = self.__extract_spatial_features(
            input_geodataframe
        )

        input_geodataframe, long_lat, geometry, target = self.__remove_target_outliers(
            input_geodataframe, long_lat, geometry, target, fit
        )
        ...
        return (
            ...
            long_lat,
            geometry,
            None if target is None else target.to_numpy(),
        )
    ...
    def fit(
        ...
        **kwargs,
        ...
        # Remove class signature from kwargs. Code similar to extract_class_args
        class_signature = inspect.signature(self.model.__init__)

        for arg, _ in class_signature.parameters.items():
            if arg in kwargs:
                kwargs.pop(arg)

        self.__set_target_transform()

        input_geodataframe, long_lat, geometry, target = self.transform(
            input_geodataframe
        )
        ...
        if (
            ...
            return (
                self.model.fit(input_geodataframe, target)
        ...
        send_coord = isinstance(
            self.model.regressor
            if isinstance(self.model, TransformedTargetRegressor)
            else self.model,
            (Kriging_Adapter, CustomModel_Adapter),
        )
        send_geom = isinstance(
            self.model.regressor
            if isinstance(self.model, TransformedTargetRegressor)
            else self.model,
            (ML_Lag_Adapter, CustomModel_Adapter),
        )
        ...
        if validation is not None:
            validation, val_long_lat, val_geometry, val_target = self.transform(
                validation, fit=False
            )
        ...
        if not hasattr(self.model, "predict") and not hasattr(
            self.model, "fit_predict"
        ):
            if self.weight_path is None:
                self.model.fit_predict(input_geodataframe, target, **kwargs)

            return self.model
        else:
            return (
                self.model.fit(input_geodataframe, target, **kwargs)
                if self.weight_path is None
                else self.model
            )
    ...
    def predict(
        ...
        **kwargs,
        ...
        input_geodataframe, long_lat, geometry, _ = self.transform(
            input_geodataframe, fit=False
        )
        send_coord = isinstance(
            self.model.regressor
            if isinstance(self.model, TransformedTargetRegressor)
        ...
        if hasattr(self.model, "predict"):
            ...
            warnings.warn(
                "The model f{self.model__class__.__name__} does not have a [predict] method",
                UserWarning,
            )
    ...
    def fit_predict(
        ...
        **kwargs,
        ...
        if (self.model is not None) and (
            isinstance(self.model, Pipeline)
            or not isinstance(
                self.model, (RegressorMixin, ClassifierMixin, ClusterMixin, str)
            )
        ):
            ...
            try:
                ...
                self.fit(
                    mapped_geodataframe if train is None else train,
                    validation=validation,
                    **kwargs,
                )
                predicted_value = self.predict(
                    mapped_geodataframe if train is None else train, **kwargs
                )

                is_custom_model = isinstance(
                    self.model.regressor
                    if isinstance(self.model, TransformedTargetRegressor)
                    else self.model,
                    CustomModel_Adapter,
                ...
                if train is None and is_custom_model:
                    model = (
                        self.model.regressor_
                        if isinstance(self.model, TransformedTargetRegressor)
                        else self.model
                    )
                    model.set_prediction(
                        X=input_geodataframe,
                        mask_outlier=self.mask_outlier,
                        y_predicted_column=predicted_column,
                        y_predicted=predicted_value,
                    )
                else:
                    _set_prediction(
                        data=input_geodataframe,
                        subset=train,
                        subset_name="train",
                        mask=self.mask_outlier,
            ...
            if not input_geodataframe[predicted_column].all():
                input_geodataframe[predicted_column] = input_geodataframe[
                    predicted_column
                ].astype(input_geodataframe[self.target_column].dtype)
